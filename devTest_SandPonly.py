...
company_only = copy.deepcopy(full)

X, Y = create_dataset(full, company_only, select_only1, HOW_MANY_TO_PICK, ATTR_PER_COMPANY, forward=FUTURE,
                       look_back=HISTORY)

b = Brain2(HISTORY, INPUT_VECTOR_DIM, COMPANIES)

x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size=0.30)

start_train = time.process_time()
b.model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=1, epochs=10)
end_train = time.process_time()
print('training and validation time: {}'.format(end_train - start_train))

model_json = b.model.to_json()
with open("model_final4.json", "w") as json_file:
    json_file.write(model_json)
b.model.save_weights('model_final4.h5')
